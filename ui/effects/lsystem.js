...
(function () {
  ...
  // Short labels: the rack lays four controls into 106 px, so the value has
  // room for about six characters. The full name is the page's context line.
  var SHORT = ['CATH', 'DRONE', 'KOTO', 'HIJAZ', '5THS', 'STORM', 'STILL'];
  var PRESETS = [
  ...
  DLNY.lsystem = {
    ...
    pages: {
      TREE: {
        context: function (s) { return PRESETS[s.preset][0]; },
        controls: function (s) {
          return [
            { label: 'Preset', obj: s, key: 'preset', min: 0, max: PRESETS.length - 1,
              fmt: F.list(SHORT), commit: 'change',
